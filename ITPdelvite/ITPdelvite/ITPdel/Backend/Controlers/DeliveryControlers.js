...
const addDelivery = async(req,res,next)=>{
    ...
    const {name,address,packSize,weight,delman,delservice,dispatchDate,expectedDate,status} = req.body;

    let delivery;

    try{
        delivery = new Delivery({name,address,packSize,weight,delman,delservice,dispatchDate,expectedDate,status});
        await delivery.save();
    }catch(err){
        console.log(err);
    }
...
const updateDelivery = async(req,res,next)=>{
    ...
    try{
        delivery = await Delivery.findByIdAndUpdate(id,
            {name:name,address:address,packSize:packSize,weight:weight,delman:delman,delservice:delservice,
                dispatchDate:dispatchDate,expectedDate:expectedDate,status:status});
